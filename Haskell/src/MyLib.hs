module MyLib (randomList, randomListRationals, randomListString, insertionSort) where
...
randomListRationals :: Int -> IO([MyRational])
randomListRationals 0 = return []
randomListRationals n = do a <- randomRIO (1,10000)
                           b <- randomRIO (1,10000)
                           ps <- randomListRationals (n-1)
                           return (makeRational a b:ps) 

randomListString :: Int -> IO([String])
randomListString 0 = return []
randomListString len = do r <- replicateM 10 $ randomRIO ('a', 'z')
                          rs <- randomListString (len-1)
                          return (r:rs)
